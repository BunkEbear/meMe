...
class oledDisp:
    ...
    def displayText(self, header, body):
        ...
        if len(bodyChunksAllOverTheFloor) >= 4:
            cv.putText(slate, bodyChunksAllOverTheFloor[3], (2, 41), cv.FONT_HERSHEY_PLAIN, 0.6, 255, 1)
        
        if len(bodyChunksAllOverTheFloor) >= 5:
            cv.putText(slate, bodyChunksAllOverTheFloor[4], (2, 48), cv.FONT_HERSHEY_PLAIN, 0.6, 255, 1)

        if len(bodyChunksAllOverTheFloor) >= 6:
            cv.putText(slate, bodyChunksAllOverTheFloor[5], (2, 55), cv.FONT_HERSHEY_PLAIN, 0.6, 255, 1)
